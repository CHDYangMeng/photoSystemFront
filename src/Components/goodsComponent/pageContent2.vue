  ...
  methods: {
    ...
    pictureTest() {
      ...
      if (this.path == '') {
        ...
        this.$http.post('http://localhost:9000/pageComponent2', {path: this.path}).then( result => {
          if (result.body.status == 200) {
            ...
            for (let i = 0; i < respResult_2.length; i++) {
              respResult.push({
                ...
                "calorie": respResult_2[i].calorie,
                "probability": respResult_2[i].probability,
              })
            }
